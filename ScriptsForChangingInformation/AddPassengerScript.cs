...
namespace ControlIndependentWork.ScriptsForChangingInformation //пространство имен
{
    ...
    class AddPassengerScript : Script
    {
        ...
        public AddPassengerScript(Passenger passenger) //параметрический конструктор
        {
            ...
            title = "Passenger passport data entry"; //присваивание полю название сценария
            text.Add("Enter the country code"); //добавлении поля текста
            text.Add("Enter the passport number"); //добавлении поля текста
            text.Add("Enter the personal number"); //добавлении поля текста
            text.Add("Enter the surname"); //добавлении поля текста
            text.Add("Enter the name"); //добавлении поля текста
            text.Add("Enter the date of birth"); //добавлении поля текста
            //если пассажир относится к классу пассажир с процентной скидкой
            if (passenger is PassengerWithPercentDiscount) text.Add("Enter the discount in percent");
            //если пассадир относится к классу пассажир с фиксированной скидкой
            else text.Add("Enter the fixed discount");
        ...
        {            
            while (true) //бесконечный цикл
            {
                ...
                {
                    //сообщение ошибки
                    errorMessage(e.Message, "Example of format for the passport number: MC1475542");
                }
                catch (InvalidPersonNumberException e) //если неверно введен личный номер
                {
                    //сообщение ошибки
                    errorMessage(e.Message, "Example of format for a personal number: 4110495G024PB5");
                }
